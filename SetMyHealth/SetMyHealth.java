...
public class SetMyHealth extends JavaPlugin {
	...
	public boolean onCommand(CommandSender sender, Command cmd,
		...
		if (commandLabel.equalsIgnoreCase("sethealth")
				|| commandLabel.equalsIgnoreCase("sh")) {
			if (args.length == 0) {
				...
				if (player.hasPermission("SetMyHealth.use.sethealth")) {
					if (args[0].matches("[0-9.]+")) {
						...
						if (amount * 2 <= player.getMaxHealth()) {
							...
							player.sendMessage("You have set your health to "
									+ ChatColor.GREEN + amount
									+ ChatColor.RESET + " hearts.");
						} else
							player.sendMessage(ChatColor.DARK_RED
									+ "That number is too high! Your max health is "
					...
					player.sendMessage(ChatColor.DARK_RED
							+ "You have insufficiant permissions!");
					...
					if (args[1].matches("[0-9.]+")) {
						...
						if (target != null) {
							...
							if (amount * 2 <= target.getMaxHealth()) {
								...
								if (player != target) {
									target.sendMessage("Your health has been set to "
											+ ChatColor.GREEN
											+ amount
											+ ChatColor.RESET
											+ " hearts by "
		...
		if (commandLabel.equalsIgnoreCase("sethunger")
				|| commandLabel.equalsIgnoreCase("shr")) {
			if (args.length == 0) {
				player.sendMessage(ChatColor.DARK_RED + "Not enough arguments!");
				return false;
			} else if (args.length == 1) {
				if (player.hasPermission("SetMyHealth.use.sethunger")) {
					if (args[0].matches("[0-9.]+")) {
						...
						if (amount <= 10) {
							...
							player.sendMessage("You have set your hunger to "
									+ ChatColor.GREEN + amount
									+ ChatColor.RESET + ".");
						} else
							player.sendMessage(ChatColor.DARK_RED
									+ "That number is too high! Your max hunger is "
					...
					if (args[1].matches("[0-9.]+")) {
						...
						if (target != null) {
							...
							if (amount <= 10) {
								...
								if (player != target) {
									target.sendMessage("Your hunger has been set to "
											+ ChatColor.GREEN
											+ amount
											+ ChatColor.RESET
											+ " by "
		...
		if (commandLabel.equalsIgnoreCase("maxhealth")
			...
			if (args.length == 0) {
				...
				if (player.hasPermission("SetMyHealth.use.maxhealth")) {
					if (args[0].matches("[0-9.]+")) {
						...
						if (amount <= getConfig().getDouble("MaxHealthLimit")) {
							...
							player.sendMessage("You have set your max health to "
									+ ChatColor.GREEN
									+ amount
									+ ChatColor.RESET + " hearts.");
						} else
							player.sendMessage(ChatColor.DARK_RED
									+ "That number is too high! The maximum amount is "
									+ ChatColor.DARK_GREEN
									+ getConfig().getInt("MaxHealthLimit")
									+ ".");
					} else
						player.sendMessage(ChatColor.DARK_RED
								+ "That is not a valid number!");
				...
				if (player.hasPermission("SetMyHealth.use.maxhealth.others")) {
					if (args[1].matches("[0-9.]+")) {
						...
						if (target != null) {
							double amount = Double.parseDouble(args[1]);
							if (amount <= getConfig().getDouble(
									"MaxHealthLimitOthers")) {
								target.setMaxHealth(amount * 2);
								if (player != target) {
									target.sendMessage("Your max health has been set to "
											+ ChatColor.GREEN
											+ amount
											+ ChatColor.RESET
											+ " hearts by "
		...
		if (commandLabel.equalsIgnoreCase("maxair")
				|| commandLabel.equalsIgnoreCase("ma")) {
			if (args.length == 0) {
				player.sendMessage(ChatColor.DARK_RED + "Not enough arguments!");
				return false;
			} else if (args.length == 1) {
				if (player.hasPermission("SetMyHealth.use.maxair")) {
					if (args[0].matches("[0-9]+")) {
						...
						if (amount1 <= getConfig().getDouble("MaxAirLimit")) {
							...
							player.sendMessage("You have set your max air to approximatly "
									+ ChatColor.GREEN
									+ amount1
							...
							player.sendMessage(ChatColor.DARK_RED
									+ "That number is too high! The maximum amount is "
									+ ChatColor.DARK_GREEN
									+ getConfig().getInt("MaxAirLimit") + ".");
					} else
						player.sendMessage(ChatColor.DARK_RED
								+ "That is not a valid number! (No decimals!)");
				} else
					player.sendMessage(ChatColor.DARK_RED
							+ "You have insufficiant permissions!");
			} else if (args.length == 2) {
				if (player.hasPermission("SetMyHealth.use.maxair.others")) {
					if (args[1].matches("[0-9]+")) {
						...
						if (target != null) {
							...
							int amount = (int) (amount1 * 19.6078431373);
							if (amount1 <= getConfig().getDouble(
									"MaxAirLimitOthers")) {
								target.setMaximumAir(amount);
								if (player != target) {
									target.sendMessage("Your max air has been set to approximatly "
											+ ChatColor.GREEN
											+ amount1
											+ ChatColor.RESET
											+ " seconds by "
									...
									player.sendMessage(ChatColor.DARK_GREEN
											...
											+ "'s"
											+ ChatColor.RESET
											+ " max air has been set to approximatly "
											+ ChatColor.GREEN + amount1
											+ ChatColor.RESET + " seconds.");
								} else
									player.sendMessage("You have set your max air to approximatly "
											+ ChatColor.GREEN
		...
		if (commandLabel.equalsIgnoreCase("setair")
				|| commandLabel.equalsIgnoreCase("sa")) {
			if (args.length == 0) {
				player.sendMessage(ChatColor.DARK_RED + "Not enough arguments!");
				return false;
			} else if (args.length == 1) {
				if (player.hasPermission("SetMyHealth.use.setair")) {
					if (args[0].matches("[0-9]+")) {
						...
						if (amount <= player.getMaximumAir()) {
							...
							player.sendMessage("You have set your air to approximatly "
									+ ChatColor.GREEN
									+ amount1
									+ ChatColor.RESET + " seconds.");
						} else
							player.sendMessage(ChatColor.DARK_RED
									+ "That number is too high! Your maximum amount is "
									+ ChatColor.DARK_GREEN
									+ Math.ceil(player.getMaximumAir() / 19.6078431373));
					} else
						player.sendMessage(ChatColor.DARK_RED
								+ "That is not a valid number! (No decimals!)");
				} else
				...
				if (player.hasPermission("SetMyHealth.use.setair.others")) {
					if (args[1].matches("[0-9]+")) {
						...
						if (target != null) {
							...
							int amount = (int) (amount1 * 19.6078431373);
							if (amount1 <= Math
									.ceil(target.getMaximumAir() / 19.6078431373)) {
								target.setRemainingAir(amount);
								if (player != target) {
									target.sendMessage("Your air has been set to approximatly "
											+ ChatColor.GREEN
											+ amount1
											+ ChatColor.RESET
											+ " seconds by "
									...
									player.sendMessage(ChatColor.DARK_GREEN
											...
											+ "'s"
											+ ChatColor.RESET
											+ " air has been set to approximatly "
											+ ChatColor.GREEN + amount1
											+ ChatColor.RESET + " seconds.");
								} else
									player.sendMessage("You have set your air to approximatly "
								...
								player.sendMessage(ChatColor.DARK_RED
										...
										+ " maximum amount is "
										+ ChatColor.DARK_GREEN
										+ Math.ceil(target.getMaximumAir() / 19.6078431373));
						} else
							player.sendMessage(ChatColor.DARK_RED
									+ "Could not find player \""
									+ ChatColor.DARK_GREEN + args[0]
			...
			if (commandLabel.equalsIgnoreCase("sethealth")
				...
				if (args.length == 0) {
					...
					player.sendMessage(ChatColor.DARK_RED + "You can only change other players stats as console!");
					return false;
				} else if (args.length == 2) {
						if (args[1].matches("[0-9.]+")) {
							Player target = player.getServer().getPlayer(args[0]);
		...
		CommandSender player = sender;
		if (commandLabel.equalsIgnoreCase("setmyhealth")
				|| commandLabel.equalsIgnoreCase("smh")) {
			...
			if (args.length > 3 || args.length == 2) {
				...
				if (player.hasPermission("SetMyHealth.use.info")) {
					player.sendMessage(ChatColor.AQUA + "|||||||" + name + " v"
							+ version + " made by Dankrushen|||||||");
					player.sendMessage(ChatColor.DARK_AQUA
							+ "Permission for all commands: SetMyHealth.use.*");
					player.sendMessage(ChatColor.DARK_AQUA
							+ "For permissions for using the command on other players just add \".others\" to the end except for \"SetMyHealth.use.info\", \"SetMyHealth.use.reload\", and \"SetMyHealth.use.set\".");
					player.sendMessage("");
					...
					player.sendMessage(ChatColor.DARK_AQUA
							+ "Permission: SetMyHealth.use.sethealth");
					player.sendMessage("");
					...
					player.sendMessage(ChatColor.DARK_AQUA
							+ "Permission: SetMyHealth.use.sethunger");
					player.sendMessage("");
					...
					player.sendMessage(ChatColor.DARK_AQUA
							+ "Permission: SetMyHealth.use.setair");
					player.sendMessage("");
					...
					player.sendMessage(ChatColor.DARK_AQUA
							+ "Permission: SetMyHealth.use.maxhealth");
					player.sendMessage("");
					...
					player.sendMessage(ChatColor.DARK_AQUA
							+ "Permission: SetMyHealth.use.maxair");
					player.sendMessage("");
					...
					player.sendMessage(ChatColor.DARK_AQUA
							+ "Permission: SetMyHealth.use.info");
					player.sendMessage("");
					player.sendMessage(ChatColor.DARK_GREEN
							+ "SetMyHealth Reload");
					player.sendMessage(ChatColor.GOLD
					...
					player.sendMessage(ChatColor.GREEN
							+ "Usage: /SetMyHealth Reload");
					player.sendMessage(ChatColor.LIGHT_PURPLE + "Alias: /SMH R");
					player.sendMessage(ChatColor.DARK_AQUA
							+ "Permission: SetMyHealth.use.reload");
					player.sendMessage("");
					player.sendMessage(ChatColor.DARK_GREEN + "SetMyHealth Set");
					player.sendMessage(ChatColor.GOLD
							+ "Description: Sets an option in the plugins config");
					player.sendMessage(ChatColor.GREEN
							+ "Usage: /SetMyHealth Set <options name> <amount>");
					player.sendMessage(ChatColor.LIGHT_PURPLE + "Alias: /SMH S");
					player.sendMessage(ChatColor.DARK_AQUA
							+ "Permission: SetMyHealth.use.set");
				} else
					player.sendMessage(ChatColor.DARK_RED
							+ "You have insufficiant permissions!");
			} else if (args.length == 1) {
				if (args[0].equalsIgnoreCase("reload")) {
					if (player.hasPermission("SetMyHealth.use.reload")) {
						reloadConfig();
						player.sendMessage(ChatColor.AQUA + name + " v"
								+ version + "'s config has been reloaded.");
					} else
						player.sendMessage(ChatColor.DARK_RED
								+ "You have insufficiant permissions!");
					...
					player.sendMessage(ChatColor.DARK_RED + "Unknown argument!");
				return false;
				...
				if (args[0].equalsIgnoreCase("set")
					...
					if (player.hasPermission("SetMyHealth.use.set")) {
						final FileConfiguration config = this.getConfig();
						if (args[1].equalsIgnoreCase("maxhealthlimit")
								|| args[1].equalsIgnoreCase("mhl")
								|| args[1]
										.equalsIgnoreCase("maxhealthlimitothers")
								|| args[1].equalsIgnoreCase("mhlo")
								|| args[1].equalsIgnoreCase("maxairlimit")
								|| args[1].equalsIgnoreCase("mal")
								|| args[1]
										.equalsIgnoreCase("maxairlimitothers")
								|| args[1].equalsIgnoreCase("malo")) {
							if (args[2].matches("[0-9.]+")) {
								...
								if (args[1].equalsIgnoreCase("maxhealthlimit")
									...
									player.sendMessage(ChatColor.AQUA
											+ "Option \"MaxHealthLimit\" in the config has been set to "
											+ ChatColor.GREEN + amount
											+ ChatColor.AQUA + ".");
								} else if (args[1]
										.equalsIgnoreCase("maxhealthlimitothers")
									...
									saveConfig();
									player.sendMessage(ChatColor.AQUA
											+ "Option \"MaxHealthLimitOthers\" in the config has been set to "
											+ ChatColor.GREEN + amount
											+ ChatColor.AQUA + ".");
								} else if (args[1]
										.equalsIgnoreCase("maxairlimit")
										|| args[1].equalsIgnoreCase("mal")) {
									config.set("MaxAirLimit", amount);
									saveConfig();
									player.sendMessage(ChatColor.AQUA
											+ "Option \"MaxAirLimit\" in the config has been set to "
											+ ChatColor.GREEN + amount
											+ ChatColor.AQUA + ".");
								} else if (args[1]
										.equalsIgnoreCase("maxairlimitothers")
									...
									saveConfig();
									player.sendMessage(ChatColor.AQUA
											+ "Option \"MaxAirLimitOthers\" in the config has been set to "
											+ ChatColor.GREEN + amount
											+ ChatColor.AQUA + ".");
								}
							} else
							...
							if (args[2].equalsIgnoreCase("true")
								...
								saveConfig();
								player.sendMessage(ChatColor.AQUA
										+ "Option \"Auto-Update\" in the config has been set to "
										+ ChatColor.GREEN + amount
										+ ChatColor.AQUA + ".");
							} else
								player.sendMessage(ChatColor.DARK_RED
										+ "That is not a valid answer! The two valid answers are \"true\" or \"false\".");
						}
					} else
						player.sendMessage(ChatColor.DARK_RED
								+ "You have insufficiant permissions!");
	...
	public void onEnable() {
		...
		if (getConfig().getBoolean("Auto-Update") == true) {
			...
			Updater updater = new Updater(this, 91493, this.getFile(),
					Updater.UpdateType.DEFAULT, true);
		} else
			this.logger
					.info("Auto-Updating is not enabled so SetMyHealth did not check for updates and stayed at version "
							+ pdfFile.getVersion());
	}
}